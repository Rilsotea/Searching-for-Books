...
const SearchBooks = () => {
  ...
  const handleSaveBook = async (bookId: string) => {
    ...
    try {
      ...
      console.log('Saving book with variables:', variables); // Log the variables
  
      const { data } = await saveBook({ variables });
  
      if (!data.saveBook || !data.saveBook.bookId) {
        throw new Error('something went wrong!');
      }
  
      setSavedBookIds((prevIds: string[]) => [...prevIds, bookToSave.bookId]);
    } catch (err) {
      console.error('Error saving book:', err); // Improved error logging
    }
  };
